...
class TQDMCallback(Callback):
    ...
    def tqdm(self, desc, total, leave, initial=0):
        ...
        """
        return tqdm(desc=desc, total=total, leave=leave, file=self.output_file, initial=initial, disable=True)
        ...
        if self.show_inner:
            if self.tqdm_inner is None:
              self.tqdm_inner = self.build_tqdm_inner(desc=desc, total=self.inner_total)
                ...
                # self.tqdm_inner.reset()
                self.tqdm_inner.refresh()  #force print final state
                self.tqdm_inner.reset()  #reuse bar
    ...
    def on_epoch_end(self, epoch, logs={}):
        if self.show_inner:
            ...
            # self.tqdm_inner.container.close()
        if self.show_outer:
    ...
    # def on_train_end(self, logs={}):
        # if self.show_outer:
            # self.tqdm_outer.container.close()
